...
build_nft_signer_prod: ## Build NFT Signer docker image for Production.
	docker build -f ./deploy/NFTSigner.Dockerfile -t docker.hicrystal.com/ud_nft_signer_prod:$(VERSION) .
...
build_nft_signer_test: ## Build NFT Signer docker image for Testing.
	docker build -f ./deploy/NFTSigner.Dockerfile -t docker.hicrystal.com/ud_nft_signer_test:$(VERSION) .
...
build_currency_signer_prod: ## Build Currency Signer docker image for Production.
	docker build -f ./deploy/CurrencySigner.Dockerfile -t docker.hicrystal.com/ud_currency_signer_prod:$(VERSION) .
...
build_currency_signer_test: ## Build Currency Signer docker image for Testing.
	docker build -f ./deploy/CurrencySigner.Dockerfile -t docker.hicrystal.com/ud_currency_signer_test:$(VERSION) .
...
build_app_prod: ## Build Application docker image for Production.
	make build_dist && docker build -f ./deploy/AppProd.Dockerfile -t docker.hicrystal.com/ud_app_prod:$(VERSION) .
...
build_app_test: ## Build Application docker image for Testing.
	make build_dist && docker build -f ./deploy/AppTest.Dockerfile -t docker.hicrystal.com/ud_app_test:$(VERSION) .
