...
export class CardService {
    ...
    public async list({ selectedPage, limit }: Pagination): Promise<CardsPage> {
        ...
        return { ...response, cards: response.cards.map((card: Card) => new CardGetters(card)) };
    };

    /** gets card by id from list of cards */
    public async getCardById(id: string): Promise<CardGetters> {
        const card = await this.card.getCardById(id);

        return new CardGetters(card);
    };
