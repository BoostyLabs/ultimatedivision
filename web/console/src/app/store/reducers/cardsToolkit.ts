...
class CardsState {
    ...
    constructor(public cardsPage: CardsPage, public card: Card) {}
}
...
const cardsState: CardsState = new CardsState(cardsPage, openedCard);

export const cardsSlice = createSlice({
    name: "cards",
    cardsState,
    reducers: {
        getCards(state: CardsState, action: PayloadAction<CardsPage>) {
            state.cardsPage = action.payload;
        },
        userCards(state: CardsState, action: PayloadAction<Card>) {
...
export const listOfCards =
    ({ selectedPage, limit }: Pagination): AppThunk =>
    async (dispatch: AppDispatch) => {
        const response = await service.list({ selectedPage, limit });

        const cards = response.cards.map(
            (card: Partial<ICard>) => new Card(card)
        );
...
export const filteredCards =
    (lowRange: string, topRange: string): AppThunk =>
    async (dispatch: AppDispatch) => {
