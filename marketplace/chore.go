...
func (chore *Chore) Run(ctx context.Context) (err error) {
	return chore.Loop.Run(ctx, func(ctx context.Context) error {
		...
		for _, lot := range lots {
			...
			privateAccountKey := chore.marketplace.config.PrivateOwnerAccountKey
			privateAccountKeyBytes, err := hex.DecodeString(privateAccountKey)
			if err != nil {
				return ChoreError.Wrap(err)
			}

			publicAccountKey := chore.marketplace.config.PublicOwnerAccountKey
			...
			_, err = transfer.FinalListing(ctx, casper.FinalListingRequest{
				...
				NFTContractHash:    fmt.Sprintf("%s%s", chore.marketplace.config.CreateListingPrefix, chore.marketplace.config.NFTContractAddress),
